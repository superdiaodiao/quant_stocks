...
from scripts.research_v14_nvmi_quarterly_reports import (
    EXPECTED_ENDS,
    _longest_chain,
    _period_columns,
    _row_value,
...
def test_nvmi_parser_selects_direct_gaap_rows() -> None:
    table = pd.DataFrame([
...
def test_nvmi_chain_requires_all_twenty_quarters() -> None:
    assert _longest_chain(EXPECTED_ENDS) == 20
    assert _longest_chain(EXPECTED_ENDS[:5] + EXPECTED_ENDS[6:]) < 20


def test_nvmi_parser_accepts_period_and_date_in_same_header() -> None:
    table = pd.DataFrame([
...
def test_nvmi_registry_is_complete_sec_only_and_identity_continuous() -> None:
    registry = pd.read_csv(
        Path("stocks_list_dir/nasdaq/nvmi_quarterly_reports.csv"),
        parse_dates=["fiscal_end", "filed_date"],
    )
    assert len(registry) == 20
    assert set(registry["ticker"]) == {"NVMI"}
    assert set(registry["cik"]) == {1109345}
