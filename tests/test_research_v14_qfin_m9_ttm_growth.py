...
def _install_source_fixtures(monkeypatch, *, value_overrides=None):
    ...
    for source_id, source in sources.items():
        ...
        downloads[qfin_growth._source_url(source)] = raw
    ...
    monkeypatch.setattr(qfin_growth, "SOURCE_DOCUMENTS", sources)
    monkeypatch.setattr(qfin_growth, "_download_bytes", fake_download)
...
def test_exact_annual_m9_ttm_arithmetic_uses_one_cny_basis() -> None:
    evidence = ttm_evidence()
    assert evidence["annual_operands_cny_thousands"] == (
        EXPECTED_ANNUAL_CNY_THOUSANDS
    )
    assert EXPECTED_TTM_CNY_THOUSANDS == {
    ...
    assert evidence["financial_age_days"] == 70
    assert evidence["guardrails"]["same_cny_basis"]
    assert evidence["guardrails"]["non_gaap_metrics_excluded"]
    assert evidence["guardrails"]["post_signal_fy2020_20f_excluded"]
...
def test_strict_facts_are_complete_direct_ttm_bundle() -> None:
    ...
    assert values["revenue_ttm"] == 12_627_349_000
    assert values["net_income_ttm"] == 2_721_042_000
    assert values["revenue_growth"] > 0.50
    assert values["net_income_growth"] < 0.15
...
def test_package_is_pit_hidden_before_6k_and_complete_at_signal() -> None:
    ...
    before = quarterly_growth_snapshot(
        facts, pd.Timestamp("2020-11-19"), maximum_age_days=150
    )
    at_signal = quarterly_growth_snapshot(
        facts, pd.Timestamp(SIGNAL_DATE), maximum_age_days=150
    )
    assert "QFIN" not in before.index
    assert at_signal.loc["QFIN", "financial_age_days"] == 70
    assert at_signal.loc["QFIN", "revenue_growth"] > 0.50
    assert at_signal.loc["QFIN", "net_income_growth"] < 0.15


def test_both_real_observations_resolve_but_fail_profit_growth_gate() -> None:
    ...
    assert set(resolution["financial_age_days"]) == {70}
...
def test_two_official_q3_exhibits_lock_identity_date_and_hash() -> None:
    assert len(SOURCE_DOCUMENTS) == 2
    assert SOURCE_DOCUMENTS["q3_2020_6k"]["accepted_at"] == (
        "2020-11-20T11:06:12Z"
    )
    assert SOURCE_DOCUMENTS["q3_2020_6k"]["expected_sha256"] == (
...
def test_source_lock_rejects_post_signal_identity_or_sha_drift() -> None:
    sources = deepcopy(SOURCE_DOCUMENTS)
    sources["q3_2020_6k"]["filed"] = "2021-01-30"
    with pytest.raises(ValueError, match="violates the PIT cutoff"):
        validate_source_lock(sources)

    sources = deepcopy(SOURCE_DOCUMENTS)
    sources["q3_2019_6k"]["document"] = "wrong.htm"
    with pytest.raises(ValueError, match="changed locked identity field document"):
        validate_source_lock(sources)

    sources = deepcopy(SOURCE_DOCUMENTS)
    sources["q3_2019_6k"]["expected_sha256"] = "bad"
    with pytest.raises(ValueError, match="invalid SHA-256"):
        validate_source_lock(sources)


def test_build_downloads_sources_and_verifies_four_statement_rows(
    ...
    assert calls == [
        qfin_growth._source_url(sources[source_id])
...
def test_build_rejects_source_row_drift(tmp_path, monkeypatch) -> None:
    _install_source_fixtures(
        monkeypatch,
        value_overrides={
            ("q3_2020_6k", "q3_m9_net_income"):
                (733_488, 1_231_724, 181_412, 2_071_606, 2_291_343, 337_477)
        },
    )
    baseline = tmp_path / "baseline.csv"
    baseline_sha = _write_audit(baseline, recovered=False)
...
def test_candidate_overlay_changes_only_bounded_qfin_rows(tmp_path) -> None:
