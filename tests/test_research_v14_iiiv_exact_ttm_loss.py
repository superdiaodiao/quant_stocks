...
from scripts.research_v14_iiiv_exact_ttm_loss import (
    CACHE,
    EXPECTED_TTM,
    _load_locked_cache,
    direct_ttm_facts,
...
def test_locked_companyfacts_operands_produce_exact_negative_ttm() -> None:
    ...
    assert evidence["net_income_ttm"] == EXPECTED_TTM == -654_000
    assert evidence["available_date"] == "2019-05-14"
    assert evidence["fiscal_end"] == "2019-03-31"
...
def test_exact_loss_resolves_age150_without_inventing_growth() -> None:
    ...
    before = quarterly_profit_ttm_snapshot(
        facts, pd.Timestamp("2019-05-13"), 150
    )
    at_signal = quarterly_profit_ttm_snapshot(
        facts, pd.Timestamp("2019-06-28"), 150
    )
    ...
    assert before.empty
    assert at_signal.loc["IIIV", "net_income_ttm"] == EXPECTED_TTM
    assert growth.empty
    assert set(facts["metric"]) == {"net_income_ttm"}


def test_build_is_blocked_research_only_and_checks_both_ages(tmp_path) -> None:
    ...
    assert resolution.loc[0, "financial_age_days"] == 45
    assert resolution.loc[0, "fiscal_age_days"] == 89
    facts = pd.read_csv(tmp_path / "strict_quarterly_facts.csv")
    assert len(facts) == 1


def test_locked_cache_rejects_tampering(tmp_path: Path) -> None:
    tampered = tmp_path / "CIK0001728688.json.gz"
