...
def test_coverage_counts_only_information_available_and_fresh():
    frame = pd.DataFrame({
        "ticker": ["A", "B"],
        "available_date": pd.to_datetime(["2026-05-01", "2025-01-01"]),
        "exact_report_date": [True, True],
    })
    audit = audit_financial_coverage(frame, ["A", "B"], pd.Timestamp("2026-07-17").date())
    assert audit["fresh_tickers"] == 1
    assert audit["fresh_coverage"] == 0.5
