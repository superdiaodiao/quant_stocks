#!/usr/bin/env python3
"""Develop a portfolio-level drawdown stop using 2020-2025 only.

The stock selector and its monthly targets stay frozen.  This experiment tests
only four standard portfolio drawdown thresholds.  A signal liquidates the
entire stock portfolio at the next trading close, and the portfolio remains in
cash until the next frozen monthly target.  The development years are training
diagnostics only and are explicitly excluded from any final win claim.
"""
...
OUTPUT_DIR = Path(
    "output/research_only/v33/portfolio_stop_development_20260830"
)
...
V32_MANIFEST = v32.RESULT_OUTPUT_DIR / "manifest.json"
...
def candidate_specs() -> list[dict]:
    return [
        {
            "key": f"portfolio_trailing_stop_{int(threshold * 100)}pct",
            "portfolio_trailing_stop_fraction": threshold,
            "signal_frequency": "daily_close",
            "execution": "next_trading_close",
            "sale_scope": "entire_stock_portfolio",
...
def _validate_sources() -> dict:
    ...
    if v30_manifest["selected_candidate"] != v30.SELECTED_CANDIDATE:
    ...
    v32_manifest = json.loads(V32_MANIFEST.read_text(encoding="utf-8"))
    if v32_manifest["observation_status"] != "BLOCKED":
        raise RuntimeError("v32 observation status changed")
    failed = sorted(
        name
        for name, passed in v32_manifest["evaluation"]["gates"].items()
        if not passed
    )
    if failed != ["drawdown_vs_nasdaq_50bps"]:
        raise RuntimeError(f"v32 failure diagnosis changed: {failed}")
    return {
        "base_candidate": v30.SELECTED_CANDIDATE,
        "v30_strategy_selection_path_complete": True,
        "v32_failed_gates": failed,
...
def freeze_protocol(path: Path = PROTOCOL_PATH) -> dict:
    path = Path(path)
    if path.exists():
        raise RuntimeError(f"v33 protocol will not be overwritten: {path}")
    runner = Path(__file__).resolve().relative_to(Path.cwd().resolve())
    protocol = {
        "schema_version": 1,
        "research_only": True,
        "stage": "V33_PORTFOLIO_STOP_DEVELOPMENT_PRECOMMITMENT",
        "status": "FROZEN_NOT_DEVELOPED",
        "frozen_at": datetime.now().astimezone().isoformat(timespec="seconds"),
        "objective": (
            "Control portfolio drawdown without changing the frozen pure-stock "
            "monthly selection path."
        ),
        ...
        "risk_policy": {
            ...
            "execution": "next trading close after signal",
        ...
        "input_bindings": {
            ...
            "v32_runner": _file_binding(
                Path("scripts/research_v32_model_isolated_stop_observation.py")
            ),
            "v32_manifest": _file_binding(V32_MANIFEST),
...
def _validated_protocol(path: Path) -> tuple[dict, str]:
    ...
    if protocol["status"] != "FROZEN_NOT_DEVELOPED":
        raise RuntimeError("v33 protocol status changed")
    if protocol["candidate_grid"] != candidate_specs():
        raise RuntimeError("v33 candidate grid changed")
    if protocol["source_diagnosis"] != _validate_sources():
        raise RuntimeError("v33 source diagnosis changed")
    if not protocol["evaluation_boundary"][
        "training_years_excluded_from_final_comparison"
    ]:
        raise RuntimeError("v33 training/final comparison boundary changed")
    for name, binding in protocol["input_bindings"].items():
        if "content_manifest_sha256" in binding:
            ...
            if actual != binding:
                raise RuntimeError(f"v33 directory binding changed for {name}")
        elif _sha256(Path(binding["path"])) != binding["sha256"]:
            raise RuntimeError(f"v33 file binding changed for {name}")
...
def _load_inputs() -> tuple[pd.DataFrame, pd.Series, pd.DataFrame]:
    ...
    observed_years = tuple(sorted(set(targets["effective_date"].dt.year)))
    if observed_years != DEVELOPMENT_YEARS:
        raise RuntimeError(f"v33 target years changed: {observed_years}")
    tickers = set(targets["ticker"].astype(str)) - {"__CASH__"}
    forbidden = sorted(tickers & FORBIDDEN_ETFS)
    if forbidden:
        raise RuntimeError(f"v33 base targets contain forbidden ETFs: {forbidden}")
...
def replay_with_portfolio_trailing_stop(
    ...
    *,
    ...
    if not 0.0 < portfolio_stop_fraction < 1.0:
        raise ValueError("portfolio stop must be between zero and one")
    ...
    targets = _target_dict(prices, target_schedule)
    ...
    for position, (current_date, daily_returns) in enumerate(returns.iterrows()):
        ...
        if target is not None:
            ...
            sold = float(position_values.sum())
        ...
        active = float(position_values.sum()) > 1e-12
        if active:
            portfolio_peak = max(portfolio_peak, nav)
            next_is_monthly_target = bool(
                next_date is not None and pd.Timestamp(next_date) in targets
            )
            if (
                not next_is_monthly_target
                and nav <= portfolio_peak * (1.0 - portfolio_stop_fraction)
            ):
                pending_exit = True
        rows.append({
...
def _canonicalize_result(
    ...
    dates = nasdaq.loc[start:end].dropna().index
    ...
    if result[["strategy", "benchmark"]].isna().any().any():
        raise RuntimeError("v33 canonical result contains missing returns")
    return result
...
def develop(
    ...
    if output_dir.exists() and any(output_dir.iterdir()):
        raise RuntimeError(f"v33 output will not be overwritten: {output_dir}")
    ...
    for spec in protocol["candidate_grid"]:
        results = {}
        for cost in COSTS:
            daily = replay_with_portfolio_trailing_stop(
                raw_close,
                nasdaq,
                targets,
                DEVELOPMENT_START,
                DEVELOPMENT_END,
                portfolio_stop_fraction=float(
                    spec["portfolio_trailing_stop_fraction"]
                ),
    ...
    for fold in protocol["walk_forward_folds"]:
        ...
        test_result = results_by_candidate[ranked_candidate][50]
        test_metrics = _period_metrics(test_result, (test_year,))
        folds.append({
            **fold,
            "selected_candidate": ranked_candidate,
            "training_gates_passed": fold_selected is not None,
            "test_excess_vs_nasdaq_50bps": test_metrics[
                "compounded_excess_vs_nasdaq"
            ],
            "test_status": (
                "PASS"
                if test_metrics["compounded_excess_vs_nasdaq"] > 0.0
                else "BLOCKED"
            ),
            "final_evidence": False,
        })
    ...
    report = {
        "schema_version": 1,
        "research_only": True,
        "stage": "V33_PORTFOLIO_STOP_DEVELOPMENT_RESULT",
