#!/usr/bin/env python3
"""Recover NVMI's SEC-filed direct GAAP quarters for v14 research only."""
...
DEFAULT_REGISTRY = Path("stocks_list_dir/nasdaq/nvmi_quarterly_reports.csv")
DEFAULT_OUTPUT_DIR = Path(
    "output/research_only/v14/nvmi_sec_quarterly_reports_2017_2021"
)
...
def _period_columns(
    ...
    *,
    ...
    if not selected:
        raise ValueError(
            f"NVMI table has no {period_phrase!r} {fiscal_end.date()} columns"
        )
    return selected
...
def _row_value(
    ...
    *,
    ...
    raise ValueError(f"expected one NVMI value for one of {labels!r}")
...
def _proves_usd_thousands(raw: bytes) -> bool:
    ...
    lower = text.casefold()
    if not (
        "nova measuring instruments" in lower or "nova ltd" in lower
    ):
        return False
...
def _parse_period(
    ...
    *,
    ...
    if not _proves_usd_thousands(raw):
        raise ValueError(f"NVMI source does not prove issuer/USD-thousands: {path}")
    ...
    if len(pairs) != 1:
        raise ValueError(
            f"expected one direct NVMI {period_phrase} pair in {path}; pairs={pairs}"
        )
    return {
...
def parse_nvmi_quarter(path: Path, fiscal_end: pd.Timestamp) -> dict[str, float]:
...
def _download(source_url: str, local_path: Path) -> None:
    ...
    if not source_url.startswith(
        "https://www.sec.gov/Archives/edgar/data/1109345/"
    ):
        raise ValueError(f"NVMI registry contains a non-SEC source: {source_url}")
...
def run(
    *, registry_path: Path = DEFAULT_REGISTRY,
    ...
    if set(registry["ticker"]) != {"NVMI"} or set(registry["cik"]) != {1109345}:
        raise ValueError("NVMI registry contains another issuer")
    if len(registry) != 20 or registry.duplicated("fiscal_end").any():
        raise ValueError("NVMI registry must contain 20 unique quarters")
    if registry.sort_values("fiscal_end")["fiscal_end"].tolist() != EXPECTED_ENDS:
        raise ValueError("NVMI registry is not the complete 2017Q1-2021Q4 chain")
    ...
    for entry in registry.sort_values("fiscal_end").itertuples(index=False):
        ...
        values = parse_nvmi_quarter(path, entry.fiscal_end)
        ...
        if not 0 <= lag_days <= 150:
            raise ValueError(f"NVMI filing is not timely: {entry.accession}")
        common = {
            "ticker": "NVMI", "fiscal_end": entry.fiscal_end,
        ...
        recovered.append({
            "ticker": "NVMI", "fiscal_end": entry.fiscal_end.strftime("%Y-%m-%d"),
    ...
    for year, annual in sorted(annual_values.items()):
        ...
        if len(year_rows) != 4 or quarter_sum != annual:
            raise RuntimeError(
                f"NVMI {year} quarters do not reconcile annual: "
    ...
    if longest != 20:
        raise RuntimeError(f"NVMI quarter chain is not continuous: {longest}/20")
    ...
    report = {
        ...
        "release_status": "BLOCKED", "ticker": "NVMI", "cik": 1109345,
        "currency": "USD", "accounting_standard": "US GAAP", "gaap_only": True,
        "issuer_identity": {
            "sec_name": "NOVA LTD.", "ticker": "NVMI",
            "continuous_2017_2021": True,
            "former_sec_name": "NOVA MEASURING INSTRUMENTS LTD",
            "name_change_effective": "2021-08-02",
        },
