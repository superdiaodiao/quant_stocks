#!/usr/bin/env python3
"""Recover ODFL bridge quarters across the SEC revenue-taxonomy transition."""
...
DEFAULT_RAW = Path(
    "cleaned_stocks_data/financial/sec_companyfacts_cache/CIK0000878927.json.gz"
)
DEFAULT_OUTPUT_DIR = Path(
    "output/research_only/v14/odfl_revenue_transition_2017_2018_bridge"
)
EXPECTED_QUARTERS = {
    pd.Timestamp("2017-03-31"): (
        pd.Timestamp("2017-01-01"), "2017-05-10", "SalesRevenueServicesNet"
    ),
    pd.Timestamp("2017-06-30"): (
        pd.Timestamp("2017-04-01"), "2017-08-07", "SalesRevenueServicesNet"
    ),
    pd.Timestamp("2017-09-30"): (
        pd.Timestamp("2017-07-01"), "2017-11-07", "SalesRevenueServicesNet"
    ),
    pd.Timestamp("2017-12-31"): (
        pd.Timestamp("2017-10-01"), "2018-02-27", "SalesRevenueServicesNet"
    ),
    pd.Timestamp("2018-03-31"): (
        pd.Timestamp("2018-01-01"), "2018-05-07", "SalesRevenueServicesNet"
    ),
    pd.Timestamp("2018-12-31"): (
        pd.Timestamp("2018-10-01"), "2019-02-27",
...
def _load_wrapper(path: Path) -> dict:
    ...
    if int(wrapper.get("cik", 0)) != 878927 or wrapper.get("symbols") != ["ODFL"]:
        raise ValueError("ODFL Company Facts wrapper identity mismatch")
    payload = wrapper.get("payload", {})
    if int(payload.get("cik", 0)) != 878927:
        raise ValueError("ODFL Company Facts payload CIK mismatch")
    if "OLD DOMINION FREIGHT LINE" not in str(payload.get("entityName", "")).upper():
        raise ValueError("ODFL Company Facts issuer mismatch")
    return wrapper
...
def _strict_quarter_rows(payload: dict) -> pd.DataFrame:
    ...
    for (expected_end, metric), concept in metric_concepts.items():
        ...
        if set(units) != {"USD"}:
            raise ValueError(f"ODFL {concept} must contain only USD facts")
    ...
    if selected.duplicated(keys, keep=False).any():
        for _, group in selected.groupby(keys):
            if len(group) > 1 and group["value"].nunique() != 1:
                raise ValueError("ODFL legacy facts contain conflicting values")
    ...
    if actual != expected:
        raise RuntimeError("ODFL legacy chain is not exactly paired 2017Q1-Q4")
...
def run(
    *, raw_path: Path = DEFAULT_RAW, output_dir: Path = DEFAULT_OUTPUT_DIR
    ...
    facts["unit"] = "USD"
    facts["source"] = "sec_companyfacts_odfl_legacy_revenue_transition"
    ...
    report = {
        ...
        "ticker": "ODFL",
        "accepted_quarter_count": 6,
        "recovered_quarters": recovered,
        "taxonomy_transition": {
            "legacy_revenue_concept": "SalesRevenueServicesNet",
            "successor_revenue_concept": (
                "RevenueFromContractWithCustomerIncludingAssessedTax"
            ),
            "restored_fiscal_period": "2017Q1-2017Q4, 2018Q1 and 2018Q4",
        },
        "raw_payload": {
            "path": str(raw_path),
        ...
        "guardrail": (
            "Only exact three-month ODFL bridge facts filed in the original "
