#!/usr/bin/env python3
"""Recover ZI 2019-2020 Q4 revenue from exact PIT duration facts."""
...
CIK = 1_794_515
RAW_PATH = Path(
    "cleaned_stocks_data/financial/sec_companyfacts_cache/CIK0001794515.json.gz"
)
RAW_SHA256 = "527b772dd047d990719bba35ba2a5d0bc4da90f25f64d17dbbf4ea85dc4e2ab1"
OUTPUT_DIR = Path("output/research_only/v14/zi_2019_q4_revenue")
CONCEPT = "RevenueFromContractWithCustomerExcludingAssessedTax"
ANNUAL_ACCESSION = "0001794515-21-000061"
NINE_MONTH_ACCESSION = "0001794515-20-000039"
...
def _one(rows: list[dict], **expected) -> dict:
    ...
    if len(matches) != 1:
        raise RuntimeError(f"ZI duration fact is not unique: {expected}")
    return matches[0]
...
def derived_q4_revenue(payload: dict) -> tuple[list[dict], list[dict]]:
    ...
    for year in (2019, 2020):
        ...
        if value != EXPECTED[year]:
            raise RuntimeError("ZI derived Q4 revenue changed")
        recovered.append(
            {
                "ticker": "ZI",
...
def recover(raw_path: Path = RAW_PATH, output_dir: Path = OUTPUT_DIR) -> dict:
    ...
    if _sha256(raw_path) != RAW_SHA256:
        raise RuntimeError("ZI raw Company Facts SHA256 changed")
    ...
    report = {
        ...
        "guardrail": (
            "Each 2019-2020 Q4 revenue equals the exact annual revenue filed "
            "on 2021-02-26 minus the matching exact nine-month revenue filed "
            "on 2020-11-13. Profit facts are deliberately untouched because "
            "pre-IPO parent attribution changes their meaning."
        ),
    }
