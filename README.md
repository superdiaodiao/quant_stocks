...
>
> 历史表现回放已经完成，但历史股票池与价格数据完整性审计仍为
> `INCOMPLETE_STRESSED`。策略在 2026-07-18 才正式冻结，冻结后的真实前向样本也尚未
> 达到生产要求。因此代码只生成推荐文件，不连接券商、不向 IBKR 自动提交订单。
...
不代表。历史结果受到参数研究过程影响，而且历史股票池价格覆盖尚未通过完整性
审计。真正生产既需要修复历史证据，也需要冻结后的前向证据。

### 是否支持 IBKR？

当前不支持自动下单。输出文件可以供人工检查，但项目没有券商提交逻辑。

截至 2026-08-03 的 paper-readiness 快照仍为 `BLOCKED`：冻结后 shadow 只有
