...
- **拆股**：Nasdaq 拆股后会按新单位改写整段历史，而本地文件只追加新行，旧单位和新
  ...
  `PROVIDER_ADJUSTMENT_DISCONTINUITY`（已存行不改写）；单日比值 ≤0.70 或 ≥1.40、
  或接近整数拆股比例的涨跌，必须有解释才能用于选股或估值；带来源的补录记录现在也
  用于选股，拆股因子允许与当天跳变相差当日真实涨跌（最多 30%）；
...
比值 ≤0.70 或 ≥1.40，或接近整数拆股比例）若发生在持有期内，MARK 会失败关闭；若
